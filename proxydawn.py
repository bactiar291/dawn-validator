...
def fetch_points(headers, proxies=None, retries=3):
    for attempt in range(retries):
        try:
            response = requests.get(api_endpoints['getPoints'], headers=headers, proxies=proxies, verify=False)
            if response.status_code == 200:
                data = response.json()
                if data.get("status"):
                ...
                print(Fore.YELLOW + f"⚠️ Attempt {attempt + 1}: Error 403 - Access forbidden. Retrying...")
                time.sleep(2)
            else:
...
def keep_alive_request(headers, email, proxies=None):
    payload = {
        "username": email,
    ...
    try:
        response = requests.post(api_endpoints["keepalive"], json=payload, headers=headers, proxies=proxies, verify=False)
        if response.status_code == 200:
            data = response.json()
            if 'message' in data:
...
def process_accounts():
    ...
    print(Fore.CYAN + f"Proxy yang digunakan: {proxies}")
    ...
    while True:
        ...
        for account in accounts:
            ...
            points = fetch_points(headers, proxies)
            total_points += points

            if points > 0:
                success = keep_alive_request(headers, email, proxies)
                if success:
